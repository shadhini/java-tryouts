...
Debug Breakpoints Console: `command` + `shift` + `fn` + `F8`

Add breakpoint at the line: `command` + `fn` + `F8`&#x20;
...
**Hot swap:**  Build after correcting code; no need to rerun code, but need to revert the last stack frame where error occurred to re-evaluate

shortcut keys: `command` + `F9`&#x20;
...
* Select the expression to be evaluated & press `option` + `command` + `fn` + `F8`&#x20;
